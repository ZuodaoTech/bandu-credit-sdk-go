...
func (c *Client) ReadSnapshots(ctx context.Context, assetID string, offset time.Time, order string, limit int) ([]*Snapshot, error) {
	...
	if err := c.Get(ctx, fmt.Sprintf("/app/accounts/%s/snapshots", c.ClientID), params, &snapshots); err != nil {
		return nil, err
	}
...
func (c *Client) ReadSnapshot(ctx context.Context, snapshotID string) (*Snapshot, error) {
	...
	if err := c.Get(ctx, fmt.Sprintf("/app/accounts/%s/snapshots/%s", c.ClientID, snapshotID), nil, &snapshot); err != nil {
		return nil, err
	}
